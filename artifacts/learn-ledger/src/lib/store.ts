  ...
  persist(
    (set) => ({
      ...
      updateAssessmentStatus: (assessmentId, status, score) =>
        set((state) => ({
          assessments: state.assessments.map((a) =>
            a.id === assessmentId
              ? { ...a, status, score, completedAt: status === 'completed' ? new Date().toISOString() : a.completedAt }
              : a
          ),
        })),
      credentials: [],
      ...
      addRoundResult: (result) =>
        set((state) => ({ roundResults: [...state.roundResults, result] })),
      resetMultiRound: () => set({ currentSkill: null, currentRound: 1, roundResults: [] }),
      ...
      resetQuiz: () =>
        set({ currentQuestionIndex: 0, answers: [], currentAssessment: null, assessmentQuestions: [] }),
      isLoading: false,

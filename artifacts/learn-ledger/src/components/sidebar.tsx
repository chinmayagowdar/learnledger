...
export default function Sidebar({ className = '' }: SidebarProps) {
  ...
  const isActive = (segment: string) => {
    ...
    return location.includes(segment);
  };
  ...
  return (
    <motion.aside
      ...
      className={`hidden md:flex fixed left-0 top-16 h-[calc(100vh-64px)] glass border-r flex-col py-8 z-30 overflow-hidden ${className}`}
    >
      <button
          ...
          return (
            <Link
              ...
              className={`flex items-center gap-3 px-4 py-3 rounded-lg transition-all duration-200 relative group ${
                active ? 'glass-dark text-primary' : 'text-foreground/70 hover:text-foreground hover:bg-muted/50'
              }`}

...
export default function QuizPage({ assessmentId }: QuizPageProps) {
  ...
  const {
    currentQuestionIndex, answers, setCurrentQuestionIndex, addAnswer, resetQuiz,
    setCurrentAssessment, setAssessmentQuestions, updateAssessmentStatus, addCredential, isLoading, setIsLoading,
  } = useLearnLedgerStore();
  ...
  if (isLoading || !assessment || assessmentQuestions.length === 0) {
    return <div className="min-h-screen flex items-center justify-center p-4"><SkeletonLoader /></div>;
  }

  if (showResults) {
    const score = answers.reduce((acc, answer, index) => answer === assessmentQuestions[index]?.correctAnswer ? acc + 1 : acc, 0);
    const credential = {
      id: `cred-${Date.now()}`,
  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-center p-4 py-12">
      <div className="w-full max-w-2xl mb-8">
        <button onClick={() => { resetQuiz(); navigate('/assessments'); }} className="text-foreground/70 hover:text-foreground transition-colors">
          ← Back
        </button>
      </div>
